...
def update_first_command_thematic_period_months(apps, schema_editor):
    TrainingTable = apps.get_model("accounts", "TrainingTable")
    tables = TrainingTable.objects.filter(
        period__section__slug="command-thematic-account",
...
class Migration(migrations.Migration):
    dependencies = [
        ("accounts", "0024_update_first_thematic_period_months"),
    ]

    operations = [
        migrations.RunPython(
            update_first_command_thematic_period_months,
